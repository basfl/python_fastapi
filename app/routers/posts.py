...
def create_posts(post: schemas.PostCreated, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    # curser.execute(""" INSERT INTO posts (title,content,published) VALUES (%s,%s,%s) RETURNING *""",
    #                (post.title, post.content, post.published))
    # new_post = curser.fetchone()
    # conn.commit()
    # dict_post=post.dict()
    # dict_post.update({"owner_id":current_user.id})
    # print(f"********{dict_post}")
    print(f'current_user.email= {current_user.id}')
...
def updated_post(id: int, updated_post: schemas.PostCreated, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
